...
class RecentProjectStore:
    """Persist recently opened projects outside the project itself."""

    def __init__(self, storage_path: Path | None = None):
        self.storage_path = storage_path or (
            Path.home() / ".dmtools" / "recent_projects.json"
        )
...
class ProjectListModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        values = (entry.name, str(entry.path.parent), format_date(entry.last_opened))
...
class ProjectPreview(QLabel):
    """Show a lightweight summary for the selected DMTools project."""
    ...
    def set_project_summary(self, project_path: Path, root_count: int) -> None:
        self.setText(
            f"{project_name(project_path)}\n\n"
            f"{root_count} root object(s)\n"
...
class FileWindow:
    ...
    def _show_project(self, path: Path, last_opened: str = ""):
        ...
        self.window.previewTitle.setText(project_name(path))
        roots = data.get("roots", [])
        object_count = sum(self._count_objects(root) for root in roots)
        rows = [
            ("Path", str(path)),
            ("Last opened", format_date(last_opened)),
    ...
    @staticmethod
    def _count_objects(item):
        count = 0 if item.get("type") == "folder" else 1
        return count + sum(
            FileWindow._count_objects(child) for child in item.get("children", [])
        )
    ...
    def browse_project(self):
        path, _ = QFileDialog.getOpenFileName(
            self.window,
            "Open DMTools Project",
            str(Path.home()),
            "DMTools projects (project.json);;JSON files (*.json)",
        )
        if not path:
            return
    ...
    def new_project(self):
        ...
        if not path:
            return
        project_path = new_project_file(path)
